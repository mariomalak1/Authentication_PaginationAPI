...
from django.contrib.auth import (
    authenticate,
    login as django_login_user,
    logout as django_logout_user,
)
...
def delete_user_if_not_himself(user_obj, delete_user):
    if user_obj != delete_user:
        ...
        return Response(
            {"error": "You can't Delete yourself"}, status=status.HTTP_403_FORBIDDEN
        )
...
def user_delete(request, user_obj):
    if request.method == "DELETE":
        delete_user = get_object_or_404(
            User, username=request.data.get("delete_username")
        )
        if user_obj.is_superuser:
            return delete_user_if_not_himself(user_obj, delete_user)
        elif (
            user_obj.is_staff
            and (not delete_user.is_staff)
            and (not delete_user.is_superuser)
        ):
            return delete_user_if_not_himself(user_obj, delete_user)
        else:
